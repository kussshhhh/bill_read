...
const ItemBox = ({ item, currency, onAssign, onEdit, onDelete, additionalCharges, subtotal, onRemoveAssignee }) => {
  ...
  // Use default value of 0 for subtotal if it's undefined or 0
  const proportion = subtotal ? (totalPrice / subtotal) : 0;
  const itemAdditionalCharges = (additionalCharges || 0) * proportion;
    ...
    >
      ...
      <div style={styles.additionalInfo}>
        ...
        <span style={styles.totalWithCharges}>
          Total: {currency}{(totalPrice + itemAdditionalCharges).toFixed(2)}
        </span>
      </div>
        ...
        {(item.assignees || []).map((person) => (
          <span key={person} style={styles.assignedPerson}>
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                onRemoveAssignee(item.name, person);
              }}
      ...
      <div style={styles.itemActions}>
        <button onClick={() => onEdit(item)} style={styles.actionButton}>
          <Edit size={16} />
        </button>
        <button onClick={() => onDelete(item.name)} style={styles.actionButton}>
          <Trash2 size={16} />
        </button>
      </div>
    </div>
